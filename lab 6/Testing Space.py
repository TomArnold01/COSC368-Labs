...
def save_to_csv(name, condition, letter, blocks, total_time):
    with open('output.csv', mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([name, condition, letter, blocks, total_time])

def updateLabel():
    global letterSub, name, Blocks   
    total_time = (time.time() - startTime) * 1000 
    save_to_csv(name, version, letterSub, Blocks, total_time) 
